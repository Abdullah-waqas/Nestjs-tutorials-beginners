...
export class CreateAppointmentGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        const active = [doctor, patient, createdBy].every((i) => i.isActive);
        if (doctor && patient && createdBy && active) {
            // NOTE: get if user has permission
            const hasUserPermission = await this.userService.hasUserPermission(createdBy.id, Number(routePermission[0]));
        ...
        // return false;
        throw new UnauthorizedException('Not an active user or the user you are trying ti');
